...
that this was a real risk this eval was specifically built to catch, the model got all 8 right,
correctly refusing every one on a test set that includes 8 questions about facts that plausibly sound
like they belong to a plan but don't (asking about Plan C's premium, inventing a "Plan G," borrowing a
detail from the wrong plan's document).
